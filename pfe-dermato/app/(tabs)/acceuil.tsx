...
import API, { analyzeImage } from '@/backend/src/api/client';
...
const makeImageUrl = (raw?: string | null): string => {
  if (!raw) {
...
export default function HomeScreen() {
  ...
  const [userPathologieIds, setUserPathologieIds] = useState<number[]>([]);
  ...
  const displayName = user?.prenom && user?.nom ? `${user.prenom} ${user.nom}` : 'Utilisateur';
  ...
  const openConseils = (pathologieIds?: number[]) => {
  if (pathologieIds && pathologieIds.length > 0) {
    router.push({
      pathname: '/(tabs)/conseil',
      params: { pathologieIds: pathologieIds.join(',') },
    } as any);
    return;
  }

  router.push('/(tabs)/conseil' as any);
};


const loadConseils = async (pathologieIds: number[]) => {
  try {
    ...
    const responses = await Promise.all(
      pathologieIds.map((id) => API.get(`/conseils/pathologie/${id}`))
    );
    ...
    const unique = Array.from(
      new Map(merged.map((c: Conseil) => [c.id, c])).values()
    ).sort((a: Conseil, b: Conseil) => (a.ordre ?? 0) - (b.ordre ?? 0));
  ...
  const loadTips = async () => {
    try {
      ...
      setTips(list.length > 0 ? list : DEFAULT_TIPS);
    } catch (err: any) {
    ...
    try {
      ...
      setRecentAnalyses(
        analyses.map((a: any) => {
          ...
          const img = makeImageUrl(a.imageMiniature ?? a.imageUrl ?? a.imagePath ?? null);
          ...
          return {
            ...
            title: formatLabel(a.title || a.classePredite || 'Analyse dermatologique'),
            confidence,
            color: getColor(confidence),
            bgColor: getBgColor(confidence),
            img,
            tag: getTag(a.tag || a.niveauUrgence),
            pathologieId: a.pathologieId ?? null,
          };
        })
      );
      ...
      const ids: number[] = Array.from(
        new Set(
          analyses
            .map((a: any) => Number(a.pathologieId))
            .filter((id: number) => !Number.isNaN(id) && id > 0)
        )
      );

      setUserPathologieIds(ids);
      await loadConseils(ids);
    ...
    <SafeAreaView style={s.safe}>
      ...
      >
        ...
        <View style={s.cardsRow}>
          <TouchableOpacity activeOpacity={0.88} style={s.cardGreen} onPress={handleDirectCamera}>
            ...
            <View style={s.cardChip}>
              <Text style={s.cardChipTxt}>📷 Photo</Text>
            </View>
          </TouchableOpacity>

          <TouchableOpacity activeOpacity={0.88} style={s.cardOrange} onPress={handleDirectGallery}>
            ...
            <View style={s.cardChip}>
              <Text style={s.cardChipTxt}>🖼️ Galerie</Text>
            </View>
          </TouchableOpacity>
        </View>
        ...
        <View style={s.alertBox}>
          ...
          <View style={{ flex: 1, marginLeft: 10 }}>
            ...
            <Text style={s.alertText}>
              Cette app ne remplace pas un avis médical professionnel.
            </Text>
          </View>
        </View>
            ...
            >
              ...
              <View style={{ flex: 1 }}>
                ...
                <View style={s.analysisMetaRow}>
                  <Text style={[s.analysisTag, { color: item.color }]}>{item.tag}</Text>
                  <Text style={s.analysisConfidence}>{item.confidence}%</Text>
                </View>
              </View>
          ...
          <View>
            <Text style={s.sectionTitle}>Conseils personnalisés</Text>
                  <Text style={s.sectionSubtitle}>
                    {userPathologieIds.length > 0
                      ? `📊 Basés sur ${userPathologieIds.length} pathologie(s) détectée(s)`
                      : '💡 Conseils généraux'}
                  </Text>
          </View>

          <TouchableOpacity style={s.seeAllBtn} onPress={() => openConseils(userPathologieIds)}>
            <Text style={s.seeAllTxt}>Voir tout</Text>
            <Svg width={14} height={14} viewBox="0 0 24 24" fill="none">
              <Path d="M9 18l6-6-6-6" stroke={C.primary} strokeWidth={2.5} strokeLinecap="round" />
            </Svg>
            ...
            return (
              <View key={conseil.id} style={s.conseilCard}>
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={s.conseilText} numberOfLines={3}>
                    {conseil.contenu}
                  </Text>

                  <TouchableOpacity
                    style={s.conseilBtn}
                    onPress={() => openConseils(userPathologieIds)}
                  >
              ...
              >
                ...
                {formError && (
                  <View style={s.errBanner}>
                    <Text style={s.errTxt}>⚠ Remplissez tous les champs obligatoires (*)</Text>
                  </View>
                )}

                {formSubmitted && (
                  <View style={s.okBanner}>
                    <Text style={s.okTxt}>✅ Formulaire soumis avec succès !</Text>
                  </View>
                )}
