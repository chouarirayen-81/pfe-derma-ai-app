...
const IconFlash = ({ on }: { on: boolean }) => (
  <Svg width={22} height={22} viewBox="0 0 24 24" fill="none">
    <Path
      ...
      stroke={on ? C.primary : '#fff'}
      strokeWidth={2}
      fill={on ? C.primary + '44' : 'none'}
...
export default function CameraScreen() {
 ...
 const handleSendToPreview = async (imageUri: string, source: 'camera' | 'gallery') => {
  try {
    setBusy(true);

    const res = await analyzeImage(imageUri);
    console.log('UPLOAD RESPONSE =', res);
    console.log('analyseId backend =', res?.analyseId);
    console.log('id backend =', res?.id);
    ...
    console.log('rawAnalyseId =', rawAnalyseId);
    console.log('imageUrl =', imageUrl);

    if (rawAnalyseId === undefined || rawAnalyseId === null || isNaN(Number(rawAnalyseId))) {
      Alert.alert('Erreur', `analyseId invalide reçu depuis le backend: ${String(rawAnalyseId)}`);
      return;
    }
    ...
    router.push({
      ...
      params: {
        imageUri,
        imageUrl,
        analyseId: String(rawAnalyseId),
        source, // ✅ important
      },
    });
  } catch (err: any) {
    console.log('Erreur upload analyse:', err?.response?.data || err?.message);
    Alert.alert('Erreur', err?.response?.data?.message || "Impossible d'envoyer l'image.");
  } finally {
    setBusy(false);
  }
};
  ...
  const handlePickImage = async () => {
    try {
      ...
      await handleSendToPreview(imageUri, 'gallery');
    } catch (err: any) {
      console.log('Erreur galerie:', err?.message);
  ...
  const handleTakePhoto = async () => {
    ...
    try {
      ...
      await handleSendToPreview(photo.uri, 'camera');
    } catch (err: any) {
      console.log('Erreur capture:', err?.message);
      ...
      <SafeAreaView style={s.safe}>
        ...
        <View style={s.permissionWrap}>
          ...
          <Text style={s.permissionText}>
            Autorise la caméra pour prendre une photo avant l’analyse.
          </Text>
          <TouchableOpacity style={s.permissionBtn} onPress={requestPermission} activeOpacity={0.85}>
            <Text style={s.permissionBtnTxt}>Autoriser la caméra</Text>
          </TouchableOpacity>
        </View>
      </SafeAreaView>
    );
  ...
  return (
    <SafeAreaView style={s.safe}>
      ...
      <View style={s.header}>
        <TouchableOpacity style={s.iconBtn} onPress={() => router.push('/(tabs)/scan')} activeOpacity={0.7}>
          <IconX />
        </TouchableOpacity>

        <Text style={s.headerTitle}>Nouvelle analyse</Text>

        <TouchableOpacity style={s.iconBtn} onPress={() => setFlash((v) => !v)} activeOpacity={0.7}>
          <IconFlash on={flash} />
        </TouchableOpacity>
      </View>
      ...
      <View style={s.viewfinder}>
        <CameraView
          ...
          facing={facing}
          flash={flash ? ('on' as FlashMode) : ('off' as FlashMode)}
      ...
      <View style={s.controls}>
        <TouchableOpacity style={s.sideBtn} onPress={handlePickImage} activeOpacity={0.8} disabled={busy}>
          <IconGallery />
        </TouchableOpacity>

        <TouchableOpacity style={s.shutterWrap} onPress={handleTakePhoto} activeOpacity={0.85} disabled={busy}>
          <View style={s.shutterOuter}>
            {busy ? <ActivityIndicator color="#fff" /> : <View style={s.shutterInner} />}
          </View>
        </TouchableOpacity>

        <TouchableOpacity style={s.sideBtn} activeOpacity={0.8} onPress={toggleCamera} disabled={busy}>
          <IconFlip />
        </TouchableOpacity>
      </View>
    </SafeAreaView>
